 ...
 *  |   |                         |                                       |   |
 *  |   |                         |                                       |
 * _memory_end |   |                         | | |   _heap_start_aligned
 * _alloc_start                            _alloc_end HEAP_START(BSS_END)
 *
...
void page_init() {
  ...
  _num_pages = (HEAP_SIZE - (_heap_start_aligned - HEAP_START)) / PAGE_SIZE -
               num_reserved_pages;
  printf("HEAP_START = %p(aligned to %p), HEAP_SIZE = 0x%x,\n"
         "num of reserved pages = %d, num of pages to be allocated for heap = "
         "%d\n",
         HEAP_START, _heap_start_aligned, HEAP_SIZE, num_reserved_pages,
         _num_pages);
  /*
